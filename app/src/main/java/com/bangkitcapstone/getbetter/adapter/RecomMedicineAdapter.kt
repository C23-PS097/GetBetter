...
class RecomMedicineAdapter(
    context: Context,
    dataMenus: List<String>,
    listener: CellClickListener
) :
    RecyclerView.Adapter<RecomMedicineAdapter.MenuViewHolder>(){
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MenuViewHolder {
        return MenuViewHolder(
            LayoutInflater.from(parent.context).inflate(
                R.layout.item_recom_medicine, parent, false
            )
        )
    }
    ...
    inner class MenuViewHolder(itemView: View) :
        ...
        fun setOnBoardingData(data: String, position: Int) {
            ...
            title.setOnClickListener {
                clickListener?.selectRecomMedicine(data)
            }
        }
    }

    interface CellClickListener {
        fun selectRecomMedicine(data: String?)
    }
